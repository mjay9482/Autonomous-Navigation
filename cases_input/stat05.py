...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 18 23:20:00 2024
...
nstatic = 1  # The bank will be static
ndynamic = 2
...
for i in range(ntotal):
    ...
    elif i == ndynamic + 1:  # Bank size
        agent_size.append(230. * 20)  # Bank is 20 times the ship size
    else:
        agent_size.append(230.)
...
for i in range(ntotal):
    if i == ndynamic + 1:  
        temp = np.random.rand(2) * 100 * agent_size[0] / agent_size[i]
        waypoints[i, :] = [temp[0], temp[1], temp[0], temp[1]]  
    else:
        reset_flag = 1
        while reset_flag == 1:
