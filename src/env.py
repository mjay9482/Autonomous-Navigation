    ...
    def __init__(self, params):
        ...
        try:
            ...
            for i, b in enumerate(boundaries):
                bound = Boundary(b[0], b[1], b[2], b[3], b[4], b[5], boundaries, i)
            ...
            if self.boundary_list:
                ...
                for j in range(len(self.boundary_list)):
                    ...
                    if self.boundary_list[j].check_agent_in_range(self, i):
                        ...
                        agent_x_vel = self.agent_list[i].curr_ss[0]
                        agent_y_vel = self.agent_list[i].curr_ss[1]
                        
                        if self.params.agent_guidance[i] is not None and 'waypoints' in self.params.agent_guidance[i]:
                            goal_wp = self.params.agent_guidance[i]['waypoints'][1]
                        else:
                            goal_wp = self.params.agent_init_state[i][4:6]

                        grad_temp = b.get_boundary_grad(agent_x, agent_y, agent_length, agent_x_vel, agent_y_vel, goal_wp)
                        grad += grad_temp
