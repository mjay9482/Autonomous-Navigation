...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  8 13:17:48 2023
...
def kcs_ode(t, v, delta_c=0, n_c=115.5/60, Fn=0.26, ypd_int=0,
        wind_flag=0, wind_speed=0, wind_dir=0,
   ...
   # n_prop = 115.5/60 # change this later (to account for speed control)
    ...
    vd[7] = nd_prop 
    vd[8] = ypd_int
    vd[9] = np.sqrt(up **2 + vp **2) - 1

    return vd

def kinematic_ode(t, v):
    ...
    xpd , ypd = R_psi @ np.array([up, vp])
    
    # xpd = up * np.cos(psi) - vp * np.sin(psi)
    # ypd = up * np.sin(psi) + vp * np.cos(psi)
